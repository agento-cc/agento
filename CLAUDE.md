...
- **Upgrade:** `agento upgrade` upgrades both the CLI package and Docker images to the latest version. Use `agento upgrade --version X.Y.Z` to pin a specific version. Image version is controlled by `AGENTO_VERSION` in `docker/.env`.
