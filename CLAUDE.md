...
- **Upgrade:** Image version is controlled by `AGENTO_VERSION` in `docker/.env`. To upgrade a deployed project: edit `.env` → `AGENTO_VERSION=X.Y.Z`, then `cd docker && docker compose pull && docker compose up -d`. The CLI itself is upgraded via `uv tool install --upgrade agento-core`.
