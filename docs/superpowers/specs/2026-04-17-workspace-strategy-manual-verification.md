...
- You are at the repository root: `/Users/dev/Projects/example/agento`.
...
```bash
cd /Users/dev/Projects/example/agento
...
ls app/code/example/workspace/                                 # must show README.md
ls app/code/example/skills/                                    # must show at least one skill directory
```
...
[ ! -L "$BUILD/modules/example/README.md" ] && echo "PASS: module file is real (modules=copy)"
...
# Assert module items symlinked under modules/example/
[ -L "$BUILD/modules/example/README.md" ] && echo "PASS: module README.md is symlink"
...
[ ! -L "$BUILD/modules/example/README.md" ] && echo "PASS: module file is real"
```
...
[ -L "$BUILD/app" ] && [ -L "$BUILD/modules/example/README.md" ] && echo "PASS: theme + modules symlinked"
...
```bash
reset_strategies
# Add scope overlay to example module that overrides README.md for dev_01
mkdir -p app/code/example/workspace/_it/_dev_01
echo "DEV_01_OVERRIDE" > app/code/example/workspace/_it/_dev_01/README.md
...
[ "$(cat $BUILD/modules/example/README.md)" = "DEV_01_OVERRIDE" ] && echo "PASS: override wins via manifest"
# Assert: file is a symlink to the _it/_dev_01 source (not base)
readlink "$BUILD/modules/example/README.md" | grep -q '_dev_01/README.md$' && echo "PASS: symlink points to _dev_01 source"

# Cleanup
rm -rf app/code/example/workspace/_it
```
