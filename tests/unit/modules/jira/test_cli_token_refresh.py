...
def _make_token(
    ...
    return Token(
        ...
        label=label,
        credentials_path=f"/etc/tokens/{agent_type}_{label}.json",
        token_limit=0,
        model=None,
        enabled=enabled,
...
class TestTokenRefresh:
    ...
    def test_refresh_primary_updates_symlink(self, mock_conn_fn, mock_config, mock_logger):
        ...
        with (
            patch(_P_GET_TOKEN, return_value=token),
            patch("sys.stdin") as mock_stdin,
            patch(_P_AUTH, return_value=_AUTH_RESULT),
            patch(_P_SAVE),
    ...
    def test_refresh_non_primary_skips_symlink(self, mock_conn_fn, mock_config, mock_logger):
        token = _make_token(is_primary=False)
        mock_conn_fn.return_value = MagicMock()
