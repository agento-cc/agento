...
from agento.modules.jira.src.crontab import CronEntry, CrontabManager
from agento.modules.jira.src.sync import JiraCronSync


def test_build_jql_without_assignee(sample_config):
    syncer = JiraCronSync(sample_config, MagicMock(), MagicMock(), logging.getLogger("test"))
    jql = syncer.build_jql()
    assert jql == 'project = AI AND status = "Cykliczne"'


def test_build_jql_with_assignee():
    ...
    syncer = JiraCronSync(config, MagicMock(), MagicMock(), logging.getLogger("test"))
    jql = syncer.build_jql()
    assert 'AND assignee = "bot@test.com"' in jql


def test_parse_issues_valid(sample_config, jira_cykliczne):
    syncer = JiraCronSync(sample_config, MagicMock(), MagicMock(), logging.getLogger("test"))
...
def test_parse_issues_skip_null_frequency(sample_config, jira_cykliczne, caplog):
    syncer = JiraCronSync(sample_config, MagicMock(), MagicMock(), logging.getLogger("test"))
...
def test_parse_issues_skip_unknown_frequency(sample_config, jira_cykliczne, caplog):
    syncer = JiraCronSync(sample_config, MagicMock(), MagicMock(), logging.getLogger("test"))
    with caplog.at_level(logging.WARNING):
        syncer.parse_issues(jira_cykliczne)
...
def test_parse_issues_empty(sample_config, jira_empty):
    syncer = JiraCronSync(sample_config, MagicMock(), MagicMock(), logging.getLogger("test"))
    entries = syncer.parse_issues(jira_empty)
    assert entries == []


def test_full_sync_dry_run(sample_config, jira_cykliczne):
    toolbox = MagicMock()
    ...
    syncer = JiraCronSync(sample_config, toolbox, crontab, logger)
...
def test_full_sync_generates_correct_crontab(sample_config, jira_cykliczne):
    toolbox = MagicMock()
    toolbox.jira_search.return_value = jira_cykliczne
...
@patch("agento.modules.jira.src.sync.get_connection")
def test_upsert_schedules_inserts(mock_get_conn, sample_config, jira_cykliczne):
...
@patch("agento.modules.jira.src.sync.get_connection")
def test_upsert_schedules_disables_removed(mock_get_conn, sample_config):
    mock_conn, mock_cursor = _mock_connection()
    mock_get_conn.return_value = mock_conn
...
@patch("agento.modules.jira.src.sync.get_connection")
def test_upsert_schedules_empty_disables_all(mock_get_conn, sample_config):
    mock_conn, mock_cursor = _mock_connection()
    mock_get_conn.return_value = mock_conn
...
@patch("agento.modules.jira.src.sync.get_connection")
def test_upsert_schedules_db_error_logged(mock_get_conn, sample_config, caplog):
    mock_conn, mock_cursor = _mock_connection()
...
def test_sync_emits_single_summary_line(sample_config, jira_cykliczne, caplog):
    toolbox = MagicMock()
    toolbox.jira_search.return_value = jira_cykliczne
...
@patch("agento.modules.jira.src.sync.get_connection")
def test_upsert_schedules_skipped_in_dry_run(mock_get_conn, sample_config, jira_cykliczne):
    toolbox = MagicMock()
    toolbox.jira_search.return_value = jira_cykliczne
    ...
    syncer = JiraCronSync(sample_config, toolbox, crontab, logging.getLogger("test"))
    syncer._do_sync(dry_run=True)
